...
class DOConv2d(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1,
                 padding=0, groups=1, dilation=1, D_mul=None, bias=True, padding_mode='zeros'):
    ...
    def __setstate__(self, state):# dummy
        super(DOConv2d, self).__setstate__(state)
        if not hasattr(self, 'padding_mode'):
            self.padding_mode = 'zeros'

    def _conv_forward(self, input, weight): # _conv_forward
        if self.padding_mode != 'zeros':
