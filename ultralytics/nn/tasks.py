...
from ultralytics.nn.modules.iaff import AFF, iAFF
from ultralytics.nn.attention.MSFAandCAFMattention import MSFN, CAFMAttention
from ultralytics.nn.attention.nlblocknd import NLBlockND
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if c2 != nc:  # if not output
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            if m in [C3_ConvNeXtv2]:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m in (CBAM, SimAM):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
