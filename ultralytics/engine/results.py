...
class Results(SimpleClass):
    ...
    def plot(
        ...
        if pred_masks and show_masks:
            ...
            '''idx = range(len(pred_masks))#绘制多种颜色
            #idx = range(5)
            annotator.masks(pred_masks.data, colors=[colors(x, True) for x in idx], im_gpu=im_gpu)'''
            import random

           # random.seed(42)  # 保证可复现，可以删除这一行实现每次都不同

            instance_colors = [
                [random.randint(0, 255) for _ in range(3)] for _ in range(len(pred_masks))
            ]

            annotator.masks(pred_masks.data, colors=instance_colors, im_gpu=im_gpu)
